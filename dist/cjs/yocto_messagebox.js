...
class YSms {
    ...
    async set_parts(parts) {
        ...
        if (retcode != yocto_api_js_1.YAPI.SUCCESS) {
            return retcode;
        }
    ...
    async send() {
        ...
        if (this._npdu > 1) {
            ...
            retcode = yocto_api_js_1.YAPI.SUCCESS;
            i = 0;
            while ((i < this._npdu) && (retcode == yocto_api_js_1.YAPI.SUCCESS)) {
                pdu = this._parts[i];
...
class YMessageBox extends yocto_api_js_1.YFunction {
    ...
    constructor(yapi, func) {
        ...
        this.OBEY_INVALID = yocto_api_js_1.YAPI.INVALID_STRING;
    ...
    async _internalEventCallback(YMessageBox_obj, str_value) {
    ...
    async fetchPdu(slot) {
        ...
        if ((binPdu).length < 8) {
            ...
            await yocto_api_js_1.YAPI.Sleep(250);
            binPdu = await this._download('sms.json?pos=' + String(Math.round(slot)) + '&len=1');
            if (!((binPdu).length >= 8)) {
     ...
     *         On failure, throws an exception or returns a negative error code.
     */
    async registerSmsCallback(callback) {
        this._smsCallback = null;
        if (callback != null) {
...
YMessageBox.OBEY_INVALID = yocto_api_js_1.YAPI.INVALID_STRING;
