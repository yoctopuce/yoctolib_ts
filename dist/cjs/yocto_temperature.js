...
class YTemperature extends yocto_api_js_1.YSensor {
    ...
    static FindTemperature(func) {
        let obj;
        obj = yocto_api_js_1.YFunction._FindFromCache('Temperature', func);
        if (obj == null) {
            obj = new YTemperature(yocto_api_js_1.YAPI, func);
            yocto_api_js_1.YFunction._AddToCache('Temperature', func, obj);
        }
        return obj;
    }
    /**
    ...
    static FindTemperatureInContext(yctx, func) {
        let obj;
        obj = yocto_api_js_1.YFunction._FindFromCacheInContext(yctx, 'Temperature', func);
        if (obj == null) {
    ...
    async loadThermistorResponseTable(tempValues, resValues) {
        ...
        idx = 0;
        while (idx < siz) {
            temp = yocto_api_js_1.YAPIContext.imm_atof(this._yapi.imm_bin2str(paramlist[2 * idx + 1])) / 1000.0;
            templist.push(temp);
            idx = idx + 1;
        }
        ...
        while (found > 0) {
            ...
            while (idx < siz) {
                ...
                if ((temp > prev) && (temp < curr)) {
                    curr = temp;
                    currRes = yocto_api_js_1.YAPIContext.imm_atof(this._yapi.imm_bin2str(paramlist[2 * idx])) / 1000.0;
                    found = 1;
                }
                idx = idx + 1;
            }
