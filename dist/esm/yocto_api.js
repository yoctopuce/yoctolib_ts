...
export class YAPIContext {
    constructor(system_env) {
        ...
        this.SUCCESS = YAPI_SUCCESS; // everything worked all right
        this.NOT_INITIALIZED = YAPI_NOT_INITIALIZED; // call yInitAPI() first !
        this.INVALID_ARGUMENT = YAPI_INVALID_ARGUMENT; // one of the arguments passed to the function is invalid
        this.NOT_SUPPORTED = YAPI_NOT_SUPPORTED; // the operation attempted is (currently) not supported
        this.DEVICE_NOT_FOUND = YAPI_DEVICE_NOT_FOUND; // the requested device is not reachable
        this.VERSION_MISMATCH = YAPI_VERSION_MISMATCH; // the device firmware is incompatible with this API version
        this.DEVICE_BUSY = YAPI_DEVICE_BUSY; // the device is busy with another task and cannot answer
        this.TIMEOUT = YAPI_TIMEOUT; // the device took too long to provide an answer
        this.IO_ERROR = YAPI_IO_ERROR; // there was an I/O problem while talking to the device
        this.NO_MORE_DATA = YAPI_NO_MORE_DATA; // there is no more data to read from
        this.EXHAUSTED = YAPI_EXHAUSTED; // you have run out of a limited resource, check the documentation
        this.DOUBLE_ACCES = YAPI_DOUBLE_ACCES; // you have two process that try to access to the same device
        this.UNAUTHORIZED = YAPI_UNAUTHORIZED; // unauthorized access to password-protected device
        this.RTC_NOT_READY = YAPI_RTC_NOT_READY; // real-time clock has not been initialized (or time was lost)
        this.FILE_NOT_FOUND = YAPI_FILE_NOT_FOUND; // the file is not found
