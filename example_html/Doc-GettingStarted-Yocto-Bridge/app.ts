...
import { YWeighScale } from '../../dist/esm/yocto_weighscale.js'

let module: YModule;
let scale: YWeighScale ;
...
async function startDemo(): Promise<void>
{
    ...
    scale = <YWeighScale>YWeighScale.FirstWeighScale();
    if(scale) {
    ...
    await scale.set_excitation(YWeighScale.EXCITATION_AC);
    let html: string = '<h1>Yocto-Bridge demo</h1>';
...
async function refresh(): Promise<void>
{
    ...
    if (await scale.isOnline()) {
        html += 'Using ' + (await module.get_serialNumber()) + ' (' + (await module.get_productName()) + ')<br>';
        html += '- scale : ' + (await scale.get_currentValue()) + (await scale.get_unit()) + '<br>';
