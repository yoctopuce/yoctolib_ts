...
{   let colorlist = {'Off': 0x000000,  'Red': 0xFF0000, 'Yellow': 0xFFFF00, 'Green': 0x00FF00, 'Cyan': 0x00FFFF, 'Blue': 0x0000FF, 'magenta': 0xFF00FF}
    ...
    { let module :YModule  = await anyLed.get_module();
      ...
      let led1 :  YColorLed  = <YColorLed>YColorLed.FindColorLed(serial+".colorLed1");
      let led2 :  YColorLed  = <YColorLed>YColorLed.FindColorLed(serial+".colorLed2");
      let html :string  = '<h1>Yocto-Color Demo</h1>';
      html +=  '<p>Using '+serial+' ('+product+')</p>'
      html += '<table>';
      let led1chooser : Chooser= new Chooser('l1output',  colorlist,(value ) => {  led1.set_rgbColor(value); });
      html += '<tr><td style="vertical-align:top">Led 1:</td><td>'+led1chooser.getHTML(await led1.get_rgbColor() )+'</td></tr>';
      let led2chooser : Chooser= new Chooser('l2output', colorlist,(value ) => {  led2.rgbMove(value,500); });
      html += '<tr><td style="vertical-align:top">Led 2:</td><td>'+led2chooser.getHTML(await led2.get_rgbColor() )+'</td></tr>';
      html += '</table>';
