...
{   let ouput : HTMLPreElement = <HTMLPreElement> document.getElementById("ouput") ;
     let unit :string  =  await sensor.get_unit();
    ouput.innerText+="Using DataLogger of " +  (await sensor.get_friendlyName())+"\n";
    let  dataset :YDataSet  = await sensor.get_recordedData(0, 0)
    ouput.innerText+="loading summary... \n"
    await dataset.loadMore()
    let summary :YMeasure  =  await dataset.get_summary()

      ouput.innerText+=" from "+ UnixTimeStampToString(await (summary.get_startTimeUTC()))
    ...
    ouput.innerText+="loading details...\n"
    ...
    {   progress = await dataset.loadMore()
        ...
        {
            ouput.innerText += " from "+ UnixTimeStampToString(await (details[i].get_startTimeUTC()))
