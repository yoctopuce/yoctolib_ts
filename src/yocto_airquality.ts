...
export class YAirQuality extends YSensor
{
    ...
    _className: string;
    _ubaIndex: number = YAirQuality.UBAINDEX_INVALID;
    _relativeIndex: number = YAirQuality.RELATIVEINDEX_INVALID;
    _aqiMode: YAirQuality.AQIMODE = YAirQuality.AQIMODE_INVALID;
    _valueCallbackAirQuality: YAirQuality.ValueCallback | null = null;
    _timedReportCallbackAirQuality: YAirQuality.TimedReportCallback | null = null;

    // API symbols as object properties
    public readonly UBAINDEX_INVALID: number = YAPI.INVALID_DOUBLE;
    public readonly RELATIVEINDEX_INVALID: number = YAPI.INVALID_DOUBLE;
    public readonly AQIMODE_RELATIVE: YAirQuality.AQIMODE = 0;
    public readonly AQIMODE_UBA: YAirQuality.AQIMODE = 1;
    public readonly AQIMODE_INVALID: YAirQuality.AQIMODE = -1;

    // API symbols as static members
    public static readonly UBAINDEX_INVALID: number = YAPI.INVALID_DOUBLE;
    public static readonly RELATIVEINDEX_INVALID: number = YAPI.INVALID_DOUBLE;
    public static readonly AQIMODE_RELATIVE: YAirQuality.AQIMODE = 0;
    public static readonly AQIMODE_UBA: YAirQuality.AQIMODE = 1;
    public static readonly AQIMODE_INVALID: YAirQuality.AQIMODE = -1;
    ...
    imm_parseAttr(name: string, val: any): number
    {
        switch (name) {
        case 'ubaIndex':
            this._ubaIndex = <number> Math.round(<number>val / 65.536) / 1000.0;
            return 1;
        case 'relativeIndex':
            this._relativeIndex = <number> Math.round(<number>val / 65.536) / 1000.0;
            return 1;
        case 'aqiMode':
            this._aqiMode = <YAirQuality.AQIMODE> <number> val;
            return 1;
        }
     ...
     */
    async get_ubaIndex(): Promise<number>
    {
        let res: number;
     ...
     */
    async get_relativeIndex(): Promise<number>
    {
        let res: number;
        if (this._cacheExpiration <= this._yapi.GetTickCount()) {
            if (await this.load(this._yapi.defaultCacheValidity) != this._yapi.SUCCESS) {
     ...
     */
    async get_aqiMode(): Promise<YAirQuality.AQIMODE>
    {
        let res: number;
        if (this._cacheExpiration <= this._yapi.GetTickCount()) {
            if (await this.load(this._yapi.defaultCacheValidity) != this._yapi.SUCCESS) {
     ...
     */
    async set_aqiMode(newval: YAirQuality.AQIMODE): Promise<number>
    {
        let rest_val: string;
     ...
     * @noreturn
     */
    async registerValueCallback(callback: YAirQuality.ValueCallback | null): Promise<number>
    {
        let val: string;
        if (callback != null) {
     ...
     * @noreturn
     */
    async registerTimedReportCallback(callback: YAirQuality.TimedReportCallback | null): Promise<number>
    {
...
export namespace YAirQuality {
    //--- (YAirQuality definitions)
    export const enum AQIMODE
    {
        RELATIVE = 0,
        UBA = 1,
        INVALID = -1
    ...
    export interface ValueCallback {(func: YAirQuality, value: string): void}

    export interface TimedReportCallback {(func: YAirQuality, measure: YMeasure): void}
