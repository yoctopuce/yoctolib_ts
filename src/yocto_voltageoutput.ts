...
export class YVoltageOutput extends YFunction
{
    ...
    public static readonly CURRENTVOLTAGE_INVALID: number = YAPI.INVALID_DOUBLE;
...
//--- (end of YVoltageOutput return codes)
    ...
    imm_parseAttr(name: string, val: any)
    {
        switch(name) {
        case 'currentVoltage':
            this._currentVoltage = <number> Math.round(<number>val * 1000.0 / 65536.0) / 1000.0;
            return 1;
     ...
     */
    async set_currentVoltage(newval: number): Promise<number>
    {
        let rest_val: string;
     ...
     */
    async get_currentVoltage(): Promise<number>
    {
        let res: number;
